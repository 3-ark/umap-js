...
// MODULE: ./node_modules/ml-matrix/matrix.js
var matrix_namespaceFn = /*#__PURE__*/__webpack_require__.cw(function(module, exports) {
...
function utils_range(n) {
    return utils_empty(n).map(function (_, i) { return i; });
}
function filled(n, v) {
    return utils_empty(n).map(function () { return v; });
}
function utils_zeros(n) {
    return filled(n, 0);
}
function utils_ones(n) {
    return filled(n, 1);
}
function linear(a, b, len) {
    return utils_empty(len).map(function (_, i) {
        return a + i * ((b - a) / (len - 1));
    });
}
function utils_sum(input) {
    return input.reduce(function (sum, val) { return sum + val; });
}
function utils_mean(input) {
    return utils_sum(input) / input.length;
}
...
function rejectionSample(nSamples, poolSize, random) {
    var result = utils_zeros(nSamples);
...
function makeHeap(nPoints, size) {
    var makeArrays = function (fillValue) {
        return utils_empty(nPoints).map(function () {
...
var matrix_values = (undefined && undefined.__values) || function(o) {
    var s = typeof Symbol === "function" && Symbol.iterator, m = s && o[s], i = 0;
    if (m) return m.call(o);
    if (o && typeof o.length === "number") return {
        next: function () {
...
var SparseMatrix = (function () {
    ...
    SparseMatrix.prototype.toArray = function () {
        var _this = this;
        var rows = utils_empty(this.nRows);
        var output = rows.map(function () {
            return utils_zeros(_this.nCols);
        });
        this.entries.forEach(function (value) {
...
function normalize(m, normType) {
    ...
    try {
        for (var _b = matrix_values(colsByRow.keys()), _c = _b.next(); !_c.done; _c = _b.next()) {
            var row = _c.value;
            _loop_1(row);
        }
    }
...
;// ./src/tree.ts
var tree_read = (undefined && undefined.__read) || function (o, n) {
    var m = typeof Symbol === "function" && o[Symbol.iterator];
    if (!m) return o;
    var i = m.call(o), r, ar = [], e;
    try {
...
var tree_values = (undefined && undefined.__values) || function(o) {
    var s = typeof Symbol === "function" && Symbol.iterator, m = s && o[s], i = 0;
    if (m) return m.call(o);
    if (o && typeof o.length === "number") return {
        next: function () {
...
function makeTree(data, leafSize, n, random) {
    ...
    var indices = utils_range(data.length);
...
function euclideanRandomProjectionSplit(data, indices, random) {
    ...
    var leftIndex = tauRandInt(indices.length, random);
    var rightIndex = tauRandInt(indices.length, random);
    rightIndex += leftIndex === rightIndex ? 1 : 0;
    rightIndex = rightIndex % indices.length;
    var left = indices[leftIndex];
    var right = indices[rightIndex];
    var hyperplaneOffset = 0;
    var hyperplaneVector = utils_zeros(dim);
    ...
    var nLeft = 0;
    var nRight = 0;
    var side = utils_zeros(indices.length);
    for (var i = 0; i < indices.length; i++) {
    ...
    var indicesLeft = utils_zeros(nLeft);
    var indicesRight = utils_zeros(nRight);
    nLeft = 0;
    nRight = 0;
...
function flattenTree(tree, leafSize) {
    ...
    var hyperplanes = utils_range(nNodes)
        .map(function () { return utils_zeros(tree.hyperplane ? tree.hyperplane.length : 0); });
    var offsets = utils_zeros(nNodes);
    var children = utils_range(nNodes).map(function () { return [-1, -1]; });
    var indices = utils_range(nLeaves)
        .map(function () { return utils_range(leafSize).map(function () { return -1; }); });
...
function recursiveFlatten(tree, hyperplanes, offsets, children, indices, nodeNum, leafNum) {
    ...
    if (tree.isLeaf) {
        ...
        (_a = indices[leafNum]).splice.apply(_a, __spreadArray([0, tree.indices.length], tree_read(tree.indices), false));
        leafNum += 1;
...
function makeLeafArray(rpForest) {
    ...
    if (rpForest.length > 0) {
        var output = [];
        try {
            for (var rpForest_1 = tree_values(rpForest), rpForest_1_1 = rpForest_1.next(); !rpForest_1_1.done; rpForest_1_1 = rpForest_1.next()) {
                var tree = rpForest_1_1.value;
                output.push.apply(output, __spreadArray([], tree_read(tree.indices), false));
            }
        }
        catch (e_1_1) { e_1 = { error: e_1_1 }; }
        finally {
...
;// ./src/nn_descent.ts
var nn_descent_values = (undefined && undefined.__values) || function(o) {
    var s = typeof Symbol === "function" && Symbol.iterator, m = s && o[s], i = 0;
    if (m) return m.call(o);
    if (o && typeof o.length === "number") return {
        next: function () {
...
function makeNNDescent(distanceFn, random) {
    return function nNDescent(data, leafArray, nNeighbors, nIters, maxCandidates, delta, rho, rpTreeInit) {
        ...
        for (var i = 0; i < data.length; i++) {
            var indices = heap_rejectionSample(nNeighbors, data.length, random);
        ...
        var sorted = deheapSort(currentGraph);
        return sorted;
    };
}
function makeInitializations(distanceFn) {
    function initFromRandom(nNeighbors, data, queryPoints, _heap, random) {
        for (var i = 0; i < queryPoints.length; i++) {
            var indices = rejectionSample(nNeighbors, data.length, random);
            for (var j = 0; j < indices.length; j++) {
                if (indices[j] < 0) {
                    continue;
                }
    ...
    function initFromTree(_tree, data, queryPoints, _heap, random) {
        for (var i = 0; i < queryPoints.length; i++) {
            var indices = searchFlatTree(queryPoints[i], _tree, random);
            for (var j = 0; j < indices.length; j++) {
                if (indices[j] < 0) {
                    return;
                }
...
function makeInitializedNNSearch(distanceFn) {
    return function nnSearchFn(data, graph, initialization, queryPoints) {
        var e_1, _a;
        var _b = getCSR(graph), indices = _b.indices, indptr = _b.indptr;
        for (var i = 0; i < queryPoints.length; i++) {
            var tried = new Set(initialization[0][i]);
            while (true) {
                var vertex = smallestFlagged(initialization, i);
                if (vertex === -1) {
                    break;
                }
                var candidates = indices.slice(indptr[vertex], indptr[vertex + 1]);
                try {
                    for (var candidates_1 = (e_1 = void 0, nn_descent_values(candidates)), candidates_1_1 = candidates_1.next(); !candidates_1_1.done; candidates_1_1 = candidates_1.next()) {
...
function initializeSearch(forest, data, queryPoints, nNeighbors, initFromRandom, initFromTree, random) {
    var e_2, _a;
    var results = makeHeap(queryPoints.length, nNeighbors);
    initFromRandom(nNeighbors, data, queryPoints, results, random);
    if (forest) {
        try {
            for (var forest_1 = nn_descent_values(forest), forest_1_1 = forest_1.next(); !forest_1_1.done; forest_1_1 = forest_1.next()) {
 ...
 * @returns True if the object is an array or a typed array.
 */
function lib_isAnyArray(value) {
    const tag = lib_toString.call(value);
    return tag.endsWith('Array]') && !tag.includes('Big');
}
//# sourceMappingURL=index.js.map
;// ./node_modules/ml-levenberg-marquardt/lib/check_options.js
...
function checkOptions(data, options) {
    ...
    else if (!lib_isAnyArray(data.x) ||
        data.x.length < 2 ||
        !lib_isAnyArray(data.y) ||
        data.y.length < 2) {
...
function getGradientDifferenceArray(gradientDifference, parameters) {
    ...
    else if (lib_isAnyArray(gradientDifference)) {
        const parLen = parameters.length;
...
function getFiller(weights, dataLength) {
    ...
    else if (lib_isAnyArray(weights)) {
...
;// ./node_modules/ml-levenberg-marquardt/lib/error_calculation.js
/**
...
const matrix_AbstractMatrix = (matrix_namespaceFn().y3);
const CHO = (matrix_namespaceFn().jy);
const matrix_CholeskyDecomposition = (matrix_namespaceFn().oN);
const matrix_DistanceMatrix = (matrix_namespaceFn().Hc);
const EVD = (matrix_namespaceFn().cg);
const matrix_EigenvalueDecomposition = (matrix_namespaceFn().hj);
const matrix_LU = (matrix_namespaceFn().LU);
const matrix_LuDecomposition = (matrix_namespaceFn().Tb);
const matrix_Matrix = (matrix_namespaceFn().uq);
const matrix_MatrixColumnSelectionView = (matrix_namespaceFn().Zm);
const matrix_MatrixColumnView = (matrix_namespaceFn().Dq);
const matrix_MatrixFlipColumnView = (matrix_namespaceFn().__);
const matrix_MatrixFlipRowView = (matrix_namespaceFn().q0);
const matrix_MatrixRowSelectionView = (matrix_namespaceFn().lh);
const matrix_MatrixRowView = (matrix_namespaceFn().pI);
const matrix_MatrixSelectionView = (matrix_namespaceFn().zC);
const matrix_MatrixSubView = (matrix_namespaceFn().zg);
const matrix_MatrixTransposeView = (matrix_namespaceFn().g6);
const NIPALS = (matrix_namespaceFn().OL);
const Nipals = (matrix_namespaceFn().ks);
const matrix_QR = (matrix_namespaceFn().QR);
const matrix_QrDecomposition = (matrix_namespaceFn().jp);
const matrix_SVD = (matrix_namespaceFn().mk);
const matrix_SingularValueDecomposition = (matrix_namespaceFn().W2);
const matrix_SymmetricMatrix = (matrix_namespaceFn().l);
const matrix_WrapperMatrix1D = (matrix_namespaceFn().KY);
const matrix_WrapperMatrix2D = (matrix_namespaceFn().dv);
const matrix_correlation = (matrix_namespaceFn().BR);
const matrix_covariance = (matrix_namespaceFn().Wu);
/* harmony default export */ const ml_matrix_matrix = ((matrix_namespaceFn().uq) ? (matrix_namespaceFn().uq) : (matrix_namespaceFn().uq));
const matrix_determinant = (matrix_namespaceFn().a4);
const matrix_inverse = (matrix_namespaceFn().DI);
const matrix_linearDependencies = (matrix_namespaceFn().Jo);
const matrix_pseudoInverse = (matrix_namespaceFn().Zi);
const matrix_solve = (matrix_namespaceFn().kH);
const matrix_wrap = (matrix_namespaceFn().LV);

;// ./node_modules/ml-levenberg-marquardt/lib/gradient_function.js
...
function gradientFunction(data, evaluatedData, params, gradientDifference, paramFunction, centralDifference) {
    ...
    const ans = matrix_Matrix.zeros(nbParams, nbPoints);
    let rowIndex = 0;
...
;// ./node_modules/ml-levenberg-marquardt/lib/step.js
...
function matrixFunction(data, evaluatedData) {
    ...
    const ans = new matrix_Matrix(m, 1);
 ...
 */
function step(data, params, damping, gradientDifference, parameterizedFunction, centralDifference, weights, jacobianFunction) {
    ...
    const cholesky = new matrix_CholeskyDecomposition(hessianApproximation);
    const perturbations = cholesky.isPositiveDefinite()
        ? cholesky.solve(jacobianWeightResidualError)
        : matrix_inverse(hessianApproximation).mmul(jacobianWeightResidualError);
    return {
        perturbations,
...
;// ./node_modules/ml-levenberg-marquardt/lib/levenberg_marquardt.js
...
var umap_read = (undefined && undefined.__read) || function (o, n) {
    var m = typeof Symbol === "function" && o[Symbol.iterator];
    if (!m) return o;
    var i = m.call(o), r, ar = [], e;
    try {
...
var umap_spreadArray = (undefined && undefined.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
...
var UMAP = (function () {
    ...
    UMAP.prototype.makeSearchFns = function () {
        var _a = makeInitializations(this.distanceFn), initFromTree = _a.initFromTree, initFromRandom = _a.initFromRandom;
        this.initFromTree = initFromTree;
        this.initFromRandom = initFromRandom;
        this.search = makeInitializedNNSearch(this.distanceFn);
    };
    UMAP.prototype.makeSearchGraph = function (X) {
        ...
        var searchGraph = new SparseMatrix([], [], [], dims);
        ...
        var transpose = matrix_transpose(searchGraph);
        return matrix_maximum(searchGraph, transpose);
    };
    UMAP.prototype.transform = function (toTransform) {
        ...
        var init = initializeSearch(this.rpForest, rawData, toTransform, nNeighbors, this.initFromRandom, this.initFromTree, this.random);
        var result = this.search(rawData, this.searchGraph, init, toTransform);
        var _a = deheapSort(result), indices = _a.indices, distances = _a.weights;
        ...
        var graph = new SparseMatrix(rows, cols, vals, size);
        var normed = normalize(graph, "l1");
        var csrMatrix = getCSR(normed);
        var nPoints = toTransform.length;
        var eIndices = reshape2d(csrMatrix.indices, nPoints, this.nNeighbors);
        var eWeights = reshape2d(csrMatrix.values, nPoints, this.nNeighbors);
    ...
    UMAP.prototype.nearestNeighbors = function (X) {
        ...
        var metricNNDescent = makeNNDescent(distanceFn, this.random);
        ...
        this.rpForest = makeForest(X, nNeighbors, nTrees, this.random);
        var leafArray = makeLeafArray(this.rpForest);
    ...
    UMAP.prototype.fuzzySimplicialSet = function (X, nNeighbors, setOpMixRatio) {
        ...
        var sparseMatrix = new SparseMatrix(rows, cols, vals, size);
        var transpose = matrix_transpose(sparseMatrix);
        var prodMatrix = pairwiseMultiply(sparseMatrix, transpose);
        var a = subtract(matrix_add(sparseMatrix, transpose), prodMatrix);
        var b = multiplyScalar(a, setOpMixRatio);
        var c = multiplyScalar(prodMatrix, 1.0 - setOpMixRatio);
        var result = matrix_add(b, c);
        return result;
    };
    ...
    UMAP.prototype.smoothKNNDistance = function (distances, k, localConnectivity, nIter, bandwidth) {
        ...
        var rho = utils_zeros(distances.length);
        var result = utils_zeros(distances.length);
        for (var i = 0; i < distances.length; i++) {
            ...
            result[i] = mid;
            if (rho[i] > 0.0) {
                var meanIthDistances = utils_mean(ithDistances);
                if (result[i] < MIN_K_DIST_SCALE * meanIthDistances) {
                    result[i] = MIN_K_DIST_SCALE * meanIthDistances;
                }
            }
            else {
                var meanDistances = utils_mean(distances.map(utils_mean));
    ...
    UMAP.prototype.computeMembershipStrengths = function (knnIndices, knnDistances, sigmas, rhos) {
        ...
        var rows = utils_zeros(nSamples * nNeighbors);
        var cols = utils_zeros(nSamples * nNeighbors);
        var vals = utils_zeros(nSamples * nNeighbors);
        for (var i = 0; i < nSamples; i++) {
            for (var j = 0; j < nNeighbors; j++) {
                var val = 0;
                if (knnIndices[i][j] === -1) {
    ...
    UMAP.prototype.initializeSimplicialSetEmbedding = function () {
        ...
        this.embedding = utils_zeros(graph.nRows).map(function () {
            return utils_zeros(nComponents).map(function () {
                return tauRand(_this.random) * 20 + -10;
            });
        });
        var weights = [];
        var head = [];
    ...
    UMAP.prototype.makeEpochsPerSample = function (weights, nEpochs) {
        var result = filled(weights.length, -1.0);
        var max = utils_max(weights);
    ...
    UMAP.prototype.prepareForOptimizationLoop = function () {
        ...
        var epochOfNextNegativeSample = umap_spreadArray([], umap_read(epochsPerNegativeSample), false);
        var epochOfNextSample = umap_spreadArray([], umap_read(epochsPerSample), false);
        this.assignOptimizationStateParameters({
    ...
    UMAP.prototype.optimizeLayoutStep = function (n) {
        ...
        for (var i = 0; i < epochsPerSample.length; i++) {
            ...
            for (var p = 0; p < nNegSamples; p++) {
                var k_1 = tauRandInt(nVertices, this.random);
...
function findABParams(spread, minDist) {
    var curve = function (params) {
        var _a = umap_read(params, 2), a = _a[0], b = _a[1];
        return function (x) {
            return 1.0 / (1.0 + a * Math.pow(x, (2 * b)));
        };
    };
    var xv = linear(0, spread * 3, 300)
        .map(function (val) { return (val < minDist ? 1.0 : val); });
    var yv = utils_zeros(xv.length).map(function (val, index) {
    ...
    var parameterValues = levenbergMarquardt(data, curve, options).parameterValues;
    var _a = umap_read(parameterValues, 2), a = _a[0], b = _a[1];
    return { a: a, b: b };
}
...
function resetLocalConnectivity(simplicialSet) {
    ...
    var transpose = matrix_transpose(simplicialSet);
    var prodMatrix = pairwiseMultiply(transpose, simplicialSet);
    simplicialSet = matrix_add(simplicialSet, subtract(transpose, prodMatrix));
    return eliminateZeros(simplicialSet);
}
function initTransform(indices, weights, embedding) {
    var result = utils_zeros(indices.length)
        .map(function (z) { return utils_zeros(embedding[0].length); });
    for (var i = 0; i < indices.length; i++) {
